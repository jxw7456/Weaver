...
const NOTION_DATABASE_ID =
  process.env.NOTION_DATABASE_ID || "1ddf46fd-48aa-806d-8a19-000b64b08ad1";
  ...
  constructor() {
    ...
    if (this.isConfigured) {
      ...
      try {
        ...
        logger.warn(
          "Notion SDK not installed. Run: npm install @notionhq/client",
        );
        this.isConfigured = false;
      }
    } else {
      logger.warn(
        "Notion API key not configured - Notion export features disabled",
      );
    }
  }
  ...
  async exportTicket(ticketId, exportedBy) {
    if (!this.isAvailable()) {
      return {
        success: false,
        error:
          "Notion integration not configured. Set NOTION_API_KEY in environment and install @notionhq/client.",
      };
    }
