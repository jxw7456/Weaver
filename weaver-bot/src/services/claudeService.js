...
const DOCUMENTATION_LINKS = {
  "App Directory": {
    helpCenter:
      "https://support-dev.discord.com/hc/en-us/sections/31439707456663-Discovery",
    apiDocs:
      "https://discord.com/developers/docs/resources/application#application-object",
  },
  "App Name Change": {
    helpCenter:
      "https://support-dev.discord.com/hc/en-us/articles/6129090215959-How-Do-I-Change-My-Bot-s-Name",
    apiDocs:
  ...
  "Premium Apps": {
    helpCenter:
      "https://support-dev.discord.com/hc/en-us/sections/17294380054935-Monetization",
  ...
  "Verification & Intents": {
    helpCenter:
      "https://support-dev.discord.com/hc/en-us/sections/5324794669207-Privileged-Gateway-Intents",
    apiDocs:
...
class ClaudeService {
  ...
  async generateInitialResponse(ticket, relevantFAQs = [], ticketHistory = []) {
    ...
    const userPrompt = this.buildTicketPrompt(
      ticket,
      relevantFAQs,
      ticketHistory,
    );
...
You are NOT resolving the ticket - you're providing a helpful first response while they wait for human support.`;
  ...
  getFallbackResponse(ticket) {
    ...
    const categoryTips = {
      "App Directory":
        "app listing details, screenshots of any issues, or your application ID",
      "App Name Change":
        "your current app name, desired new name, and application ID",
      "API & Gateway":
        "error codes, API responses, relevant code snippets, or gateway event logs",
      "Developer Community Perks":
        "your developer profile or any eligibility questions",
      ...
      "Teams & Ownership":
        "team ID, current ownership details, or transfer requirements",
      "Verification & Intents":
        "your application ID, current verification status, or intent requirements",
      Webhooks: "webhook URL issues, delivery failures, or payload examples",
    };

    const tip =
      categoryTips[ticket.category] ||
      "any relevant details, screenshots, or error messages";
    ...
    try {
      const response = await axios.post(
        CLAUDE_API_URL,
        {
          ...
          max_tokens: 256,
          system:
            "You are a support ticket management assistant. Generate a brief, professional escalation notice for staff about an unattended ticket. Be concise - 2-3 sentences max.",
          messages: [
            {
              role: "user",
