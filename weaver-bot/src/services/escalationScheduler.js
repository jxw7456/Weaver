...
class EscalationScheduler {
  ...
  startScheduler() {
    ...
    logger.info(
      "Escalation Scheduler started. Checking for stale tickets every hour.",
    );
  }
  ...
  async escalateTicket(ticket) {
    try {
      ...
      const escalationMessage =
        await claudeService.generateEscalationReminder(ticket);

      // Get the ticket channel/thread
      const ticketChannel = await this.client.channels
      ...
      if (logChannel) {
        const logEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `Ticket #${ticket.id} has been waiting over 24 hours without staff response.`,
          )
          .addFields(
